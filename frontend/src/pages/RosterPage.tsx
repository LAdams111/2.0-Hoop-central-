    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col items-center gap-4 text-center md:flex-row md:text-left">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Team roster</p>
        </div>
        <div className="ml-auto">
          <label className="text-sm text-muted-foreground">Select season</label>
          <select
            ...
            className="mt-1 block rounded-lg border border-input bg-background px-3 py-2 text-foreground focus:border-primary focus:outline-none"
          >
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={handleToggleFavoriteTeam}>
          <Heart className={`mr-2 h-4 w-4 ${isFavTeam ? "fill-primary text-primary" : ""}`} /> Favorite team
        </Button>
      </div>
      ...
      {!isLoading && players.length === 0 && (
        <p className="py-12 text-center text-muted-foreground">No players on roster.</p>
      )}
    </div>
  );
}
